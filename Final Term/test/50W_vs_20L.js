...
contract("Testing Mayor", accounts => {
  ...
  it("Should test compute_envelope", async function(){
    ...
    for(i = 0; i < nayVoters; i++){
      nayVotersArray[i] = await instance.compute_envelope(1,false, web3.utils.toWei("1", "ether"), {from: accounts[yayVoters+i]});
    }
  })
  ...
  it("Should test open_envelope", async function(){
    ...
    for(i = 0; i < nayVoters; i++){
      await instance.open_envelope(1,false, {from: accounts[yayVoters+i], value: web3.utils.toWei("1", "ether")})
    }
  })
